...
.config(config)
	function config($routeProvider) {
		$routeProvider.when('/info', {
			templateUrl : 'nm-info-tpl.html',
			controller:'nmController'
		}).when('/otherinfo', {
			templateUrl : 'nm-otherinfo-tpl.html'
		}).otherwise({
			redirectTo : '/info'
		});
	}
	...
	function nmController($scope, dataService,$routeParams,$filter){
		...
		function save(tradpiny){
			...
			Torch.dataSubmit(params).then(function(data){
				...
				Torch.redirect("./nm-setup.html?gid="+gid+"&nameid="+nameid+'#/otherinfo');
			});
		}
...
(function(){
    ...
    function nmOtherinfoController($scope, dataService,$routeParams){
        ...
        //返回上一步
        function back(){
        	Torch.redirect("./nm-setup.html?gid="+Torch.getParam('gid')+"&nameid="+Torch.getParam('nameid')+'#/info');
        }
        //投资人分页
        ...
        function approval(){
        	Torch.validateForm('nameOtherInfo').then(function(){
        		var config = {
                    ...
                    Torch.openwin(config).then(function(data){
                    	if(data === true){
                    		Torch.confirm('提交成功', function(){
                    			Torch.redirect('./nm-setup.html');
                    		});
                    	}
                    });
        	});
