...
.config(config)
	function config($routeProvider) {
		$routeProvider.when('/info', {
			templateUrl : 'nm-adjust-info-tpl.html',
			controller:'nmController'
		}).when('/otherinfo', {
			templateUrl : 'nm-adjust-otherinfo-tpl.html',
			controller:'nmOtherinfoController'
		}).otherwise({
			redirectTo : '/info'
		});
	}
	...
	function nmController($scope, dataService,$routeParams,$filter){
		...
		function query(){
			...
			if(gid && nameid){
				...
				var entparams={
						url:"/torch/service.do?fid=nameAdjustEntQuery",
						param:{
							nameAdjustEntQueryForm: {
								nameid: nameid
							}
						}
				};
				Torch.dataSubmit(entparams).then(function(data){
					vm.nmDetail = data.nameAdjustEntQueryForm || {};
				})
			}
		}
		...
		function checkRule(){
			var gid = Torch.getParam('gid');
			vm.nmInfoForm.appCode = 'companyNameTrim';
			var params = {
		...
		function save(tradpiny){
			...
			Torch.dataSubmit(params).then(function(data){
				...
				Torch.redirect("./nm-adjust.html?gid="+gid+"&nameid="+nameid+'#/otherinfo');
			});
		}
...
(function(){
    ...
    function nmOtherinfoController($scope, dataService,$routeParams){
        ...
        //返回上一步
        function back(){
        	Torch.redirect("./nm-adjust.html?gid="+Torch.getParam('gid')+"&nameid="+Torch.getParam('nameid')+'#/info');
        }
        //投资人分页
        ...
        function investmentEdit(data){
            ...
            var config = {
                url:'../inv/inv-info.html?gid='+gid+'&editAdd=0&nameid='+nameid+"&invid="+data,
                title:'编辑投资人'
            }
            Torch.openwin(config).then(function(){
                Torch.info('保存成功');
